...
export const useObjectsSlice = create((set, get) => ({
  ...
  getStateByGateId(gateId, nextState) {
    const gates = get().GATES
    const gate = gates[gateId]
    const result = {}
  ...
  addGate(input) {
    ...
    const newGate = {
      // id: v7(),
      id: `${input.type}_${count + 1}`,
      name: `${input.type}_${count + 1}`,
      type: input.type,
    ...
    switch (input.type) {
      ...
      case XOR_GATE:
        ...
        newGate.outputs[OUT_Q] = { status: GATE_FUNCTIONS[input.type]({})[OUT_Q], destGate: [] }
        newGate.delay = 2
      ...
      case SWITCH:
        ...
        newGate.delay = 5
        event = { gateId: newGate.id, time: get().TIME, gateState: GATE_FUNCTIONS[input.type]({}) }
        break
    }
