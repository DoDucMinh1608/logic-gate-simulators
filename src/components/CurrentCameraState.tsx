...
function CurrentCameraState(): ReactNode {
  ...
  const angles = useMemo(() => getDegrees(camera?.direction!), [camera?.direction])
    ...
    <div className="font-mono absolute  left-0 top-0 translate-x-2 flex bg-[rgba(255,255,255,0.5)] p-1 gap-5">
      <div>
        ...
        <p className="pl-5 grid grid-cols-2 gapx-5">
          <span>x: </span><span>{camera?.position.x.toFixed(2)}</span>
          <span>y: </span><span>{camera?.position.y.toFixed(2)}</span>
          <span>z: </span><span>{camera?.position.z.toFixed(2)}</span>
        </p>
      </div>
      <div>
        Direction <br />
        <div className="pl-5 grid grid-cols-2 gapx-5">
          <span>yaw:</span><span>{angles.yaw}°</span>
          <span>pitch:</span><span>{angles.pitch}°</span>
        </div>
      </div>
    </div>
  )
